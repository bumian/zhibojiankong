...
from instar import *
from live_platform.Douyu import Douyu
import time
...
def run():
    ...
    follow_list = douyu.traverse_living("live",30000)
    send_list = []
    pf = "douyu"
    ...
    for live in follow_list:
        feed = live
        if feed["show_status"] == "1" and feed["cate_id"] != 207:#and feed["game_name"] != u"企鹅直播":
            uid = feed["owner_uid"]
            if str(uid) not in monoit_list:
                continue
            if pf+"-"+str(uid) not in sent_uid and uid not in send_list:
                live_id = feed["room_id"]
